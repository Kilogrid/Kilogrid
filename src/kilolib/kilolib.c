...
void message_rx_dummy(IR_message_t *m, distance_measurement_t *d) { }
IR_message_t *message_tx_dummy() { return NULL; }
...
IR_message_t rx_msg;                  // message being received
...
ISR(TIMER0_COMPA_vect) {
    ...
    if(!rx_busy && tx_clock>kilo_tx_period && kilo_state == RUNNING) {
       IR_message_t *msg = kilo_message_tx();
        if (msg) {
            if (message_send(msg)) {
        ...
        if (timer <= rx_bitcycles/2 || timer >= rx_bitcycles*9+rx_bitcycles/2) {
            ...
            if (bitindex <= 7) { // Data bit received.
                ...
                if (rx_leadingbyte) {
                    ...
                    rx_byteindex++;
                    if (rx_byteindex == sizeof(IR_message_t)) {
                        rx_timer_off();
                        rx_leadingbyte = 1;
                        rx_busy = 0;
