...
void FTDIConnection::startCellBootload(uint8_t pages_total) {
...
void FTDIConnection::sendCellBootpage(QByteArray bootpage, uint8_t bootpage_num) {
                ...
                switch(buf[offset+1]) {
                    case SERIAL_PACKET_TRACKING:
                        ...
                        for(uint8_t i = 0; i < tracking_packet->number_of_items; i++) {
                            ...
                            tracking_packet->items[i].cell = buf[offset+6 + i*sizeof(tracking_item_t)];
                    ...
                    case SERIAL_PACKET_MODULE_BOOTLOAD_GET_PAGE:
                        offset += 4;
                        emit requestCellBootPage(buf[2]);
                        break;
