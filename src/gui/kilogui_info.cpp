...
namespace {
    ...
    const QString TAG_FOR_CELL_PROGRAM      = "last_module_controller_program";
...
void KiloGUI_Info::updateInfoFile(){
    ...
    if(isAvailable(MODULE_PROGRAM)) {
        xml_writer.writeTextElement(TAG_FOR_CELL_PROGRAM, module_controller_path);
    }
    if(hasBaudRate()) {
...
void KiloGUI_Info::readInfoFile(){
    ...
    while(!xml_reader.atEnd()){
        ...
        if(xml_reader.name() == TAG_FOR_CONFIGURATON_FILE){
            ...
            xml_reader.readNext();
        } else if (xml_reader.name() == TAG_FOR_CELL_PROGRAM) {
            xml_reader.readNext();
...
void KiloGUI_Info::setCurrent(file_type type, QString path)
{
    ...
    case CONFIGURATION_FILE:
        ...
        kilobot_controller_path = getWorkingDirectoryPath() + "/kilobot/build/kilobot.hex";
