      ...
      connected(false)                                                    // make KiloGUI disconnected to Kilogrid
{
    ...
    main_grid_layout->addWidget(createDebugWindow(), 0, 3, 3, 3); // the debug window occupies the cells 3,0 to 3,3 in the grid layout
...
//    serial_conn = new SerialConnection();
    ...
    */
    ...
    connect(ftdi_conn, SIGNAL(requestCellBootPage(quint8)), this, SLOT(sendCellBootPage(quint8)));
...
//    } else {    
        ...
        event->accept();
        //    }
    if(cell_program != NULL) delete cell_program;
...
QGroupBox *KiloWindow::createKilogridFileInput() {
    QSignalMapper *mapper = new QSignalMapper(this);
    QGroupBox *box = new QGroupBox("Upload Module Program");
    QGridLayout *layout = new QGridLayout;
...
QGroupBox *KiloWindow::createKilogridCommands() {
    ...
    cell_setup_button = new QPushButton("Setup");
    disable_when_run.push_back(cell_setup_button);
    layout->addWidget(cell_setup_button, 0, 0);
    mapper->setMapping(cell_setup_button, (int)KILOGRID_SETUP);
    QObject::connect(cell_setup_button, SIGNAL(clicked()), mapper, SLOT(map()));
    ...
    cell_run_button = new QPushButton("Run");
    layout->addWidget(cell_run_button, 2, 0);
    mapper->setMapping(cell_run_button, (int)KILOGRID_RUN_EXP);
    QObject::connect(cell_run_button, SIGNAL(clicked()), mapper, SLOT(map()));
...
void KiloWindow::updateStatus() {
    ...
    if (str.startsWith("Connect")) { // check if the connection with the dispatcher was established
        ...
        cell_setup_button->setEnabled(true);
        cell_run_button->setEnabled(true);
...
//        if (kilogrid_program_file.isEmpty())
        ...
        cell_setup_button->setEnabled(false);
        cell_run_button->setEnabled(false);
...
void KiloWindow::uploadKilogridProgram() {
    ...
    if (!app_info->isAvailable(target)) {
        ...
        if(cell_program != NULL) delete cell_program;
        cell_program = new ProgramStore(app_info->getLast(target));
        kilogrid_command_active = true;

        qDebug() << "Upload module program:" << app_info->getLast(target);

//        if (device == 0)
        ftdi_conn->startCellBootload(cell_program->getNumBootPages());
        ftdi_conn->read();
    }
}

void KiloWindow::sendCellBootPage(quint8 bootpage_num) {
    ftdi_conn->sendCellBootpage(cell_program->getBootPage(bootpage_num), bootpage_num);
    ftdi_conn->read();

    if(bootpage_num == cell_program->getNumBootPages()-1) {
...
void KiloWindow::kilogrid_disable_command(){
    ...
    sendKilogridCommand(KILOGRID_IDLE); // IDLE command puts the cell in IDLE state (toggle between active commands and this state)
    ...
    if(exp_running) {
        cell_run_button->setText("Wait for it...");
        cell_run_button->setEnabled(false);
        timer->stop();
    }
}
...
void KiloWindow::sendKilogridCommand(int type_int){
    ...
    if(type == KILOGRID_SETUP) {
        onExperimentStarted();
        cell_run_button->setEnabled(false);
        cell_setup_button->setText("Sending...");
        sendKilogridConfiguration();
        cell_setup_button->setText("Setup");
        onExperimentStopped();
        cell_setup_button->toggle();
    }
