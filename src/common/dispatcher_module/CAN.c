 ...
 * The ID field of a CAN message encodes the coordinates of the cell in Kilogrid in the following manner: ID<10:0> = [y<4:0> + 1][x<4:0> + 1][destination_type_t]
 * For example, when accessing cell with coordinates (x = 0, y = 0) with individual addressing, ID<10:0> = 00001 00001 0.
 * Zeros in the ID field are reserved for broadcasting to all cells (00000 00000 0), cells in a row (yyyyy 00000 0), or cells in a column (00000 xxxxx 0).
 * @param m
 * @param coord
 * @param dest_type
 */
uint16_t get_CAN_ID_from_Kilogrid_address(kilogrid_address_t coord, destination_type_t dest_type){
	...
	switch(coord.type){
		...
		case ADDR_ROW:
			...
			ID_MSB = coord.y + 1; // y coordinate increased, so that y = 0 is reserved to column broadcast messages, and individual cells with coord_y = 0 correspond to an index of 1 in the CAN ID
			break;
		case ADDR_COLUMN:
			ID_LSB = coord.x + 1;
		...
		// Individual addressing and messages to the dispatcher are processed in the same way. When a cell wants to send a message to the dispatcher, it appends its x and y coordinates to the message to be sent. Only the dest_type differs (appended at the LSB of the CAN_ID)
...
void init_CellCAN(uint8_t x_coord, uint8_t y_coord){
	...
	// set filters for current device
		...
		cell_address.x = x_coord;
		cell_address.y = y_coord;
		...
		// set filters for every type of addressing
  		for(uint8_t k = ADDR_BROADCAST; k <= ADDR_INDIVIDUAL; k++){
			cell_address.type = k;
			mcp2515_set_filter(k, get_CAN_ID_from_Kilogrid_address(cell_address, TO_CELL) ); // receive messages to be sent to the cells, on the basis of the coordinates of current cell

		}

		// setting filters 4 and 5 as well to a valid ID (not used, but have to be set properly)
		cell_address.type = ADDR_BROADCAST;
		mcp2515_set_filter(4, get_CAN_ID_from_Kilogrid_address(cell_address, TO_CELL) ); // receive messages to be sent to the cells, on the basis of the coordinates of current cell

		cell_address.type = ADDR_BROADCAST;
		mcp2515_set_filter(5, get_CAN_ID_from_Kilogrid_address(cell_address, TO_CELL) ); // receive messages to be sent to the cells, on the basis of the coordinates of current cell

	#elif defined(DISPATCHER)

		// Note: does not seem to filter out addresses to the attention of the cells.
...
uint8_t CAN_message_tx(CAN_message_t *message, kilogrid_address_t dest) {
	...
	if(dest.type == ADDR_DISPATCHER){
		...
		message->id = get_CAN_ID_from_Kilogrid_address(dest, TO_CELL);
	}
	...
	if(msg_success){
		cell_CAN_message_tx_success(); // callback
	}
...
void serialize_tracking_message(CAN_message_t* msg, uint8_t subcell_id, tracking_user_data_t* data) {
	...
	subcell_id &= 0b00000011;
	msg->data[0] = CAN_TRACKING_KILOBOT | (subcell_id << 6);
