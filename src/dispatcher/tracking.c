 ...
 */
inline void tracking_message_received(kilogrid_address_t address, uint8_t sc, tracking_user_data_t* data) {
	for(uint8_t i = 0; i<TRACKING_MSG_USER_DATA; i++) {
		serial_packet_tracking.items[current_tracking_index].cell_data[i] = data->byte[i];
	}
	
	serial_packet_tracking.items[current_tracking_index].cell_address = address;
	serial_packet_tracking.items[current_tracking_index].sc = sc;
...
void init_serial_packet_tracking(serial_packet_tracking_t *s){
	...
	for(i = 0; i < TRACKING_ITEM_BUFFER_SIZE; i++){
		s->items[i].cell_address.x = 0; 
		s->items[i].cell_address.y = 0, 
		s->items[i].cell_address.type = 0;

		for(j = 0; j<TRACKING_MSG_USER_DATA; j++ ) {
			s->items[i].cell_data[j] = 0;
		}
